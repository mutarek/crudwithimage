...
<div class="row">
    ...
    @csrf 
    <input type="hidden" name="id" id="" value="{{$editablepost->id}}">
    <div class="form-outline mb-3">
            @error('title')
            <span class="text-danger">{{$message}}</span>
            @enderror
        <input name="title" type="text" id="form4Example1" class="form-control" value="{{$editablepost->title}}"/>
        <label class="form-label" for="form4Example1">Title</label>
    </div>
    <div class="form-group mb-2">
        <img src="{{asset($editablepost->image)}}" alt="" height="50px">
    </div>
    <div class="form-outline mb-3">
             @error('filetoupload')
            <span class="text-danger">{{$message}}</span>
